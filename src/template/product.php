    ...
    <article class="flex flex-col sm:flex-row items-center justify-center rounded-md p-5 shadow-md bg-white h-min w-full max-w-xs sm:max-w-xl">
        <?php foreach ($templateParams["prodottoid"] as $prodottoid): ?>
            ...
            <div class="text-center w-full">
                ...
                <div id="fixed-div" class="flex flex-row w-full fixed bottom-0 left-0 sm:left-auto sm:bottom-auto sm:static sm:rounded-xl overflow-hidden">
                    <button type="button" class="w-3/4 p-4 bg-blue text-white hover:bg-darkBlue" data-codice-articolo="1" onclick="addToCart(this)">Aggiungi al carrello</button>
                    <button type="button" class="w-1/4 flex justify-center items-center bg-darkPink hover:border-darkBlue" data-codice-articolo="1" onclick="addToFavorites(this)">
