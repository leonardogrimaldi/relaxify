<div class="grow flex flex-col lg:flex-row">
    ...
    <section class="flex flex-col lg:items-center lg:w-2/4">
        <h2 class="text-center text-lg py-3 w-full" itemprop="name">Prodotti preferiti</h2>
            ...
            <?php else: ?>
                <?php foreach ($templateParams["prodottipreferiti"] as $prodottopreferito): ?>
                    <article class="flex flex-row items-center m-2 mt-1 rounded-sm h-32">
                        <a class="w-1/5" href="prodotto.php?id=<?php echo $prodottopreferito['prodottoID'] ?>">
                            <img class="aspect-square rounded" height="150" width="150" src="<?php echo IMG_ROOT . $prodottopreferito["immagine"] ?>" alt="<?php echo $prodottopreferito["nome"] ?>">
                        </a>
                        <div class="flex flex-col h-full mx-2 justify-center w-full">
                            <hgroup>
                                <h3 class="text-lg"><?php echo $prodottopreferito["nome"] ?></h3>
                            </hgroup>
                            <p class="mt-2 line-clamp-3"><?php echo $prodottopreferito["descrizione"] ?></p>
                        </div>
                        <div class="flex flex-col h-full justify-center items-center mr-2">
                            <data value="<?php echo $prodottopreferito["prezzo"] ?>"><span class="font-bold"><?php echo $prodottopreferito["prezzo"] ?>€</span></data>
